...
function calcularHash(contrato: ObraNormalizada, isObra: boolean): string {
  const canonico = [
    ...
    String(contrato.valor_pagado ?? ""),
    String(contrato.valor_pendiente_ejecucion ?? ""),
    contrato.url_secop,
    isObra ? "obra" : "no-obra",
  ].join("|");
...
function construirFila(contrato: ObraNormalizada): ObraRow {
  ...
  return {
    ...
    valor_pagado: contrato.valor_pagado,
    valor_facturado: contrato.valor_facturado,
    valor_pendiente_ejecucion: contrato.valor_pendiente_ejecucion,
    valor_pendiente_pago: contrato.valor_pendiente_pago,
