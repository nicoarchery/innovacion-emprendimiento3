...
import type { ReporteFila } from "@/components/mapa/reportes-comunes";
...
export function ReporteCiudadanoModal({
  ...
  const esEdicion = reporte !== undefined;
  const { toast } = useToast();
  const [estadoTerreno, setEstadoTerreno] = useState(reporte?.estadoTerreno ?? "ejecucion");
  const [avanceObservado, setAvanceObservado] = useState(reporte?.avanceObservado ?? 50);
  const [calificacion, setCalificacion] = useState(reporte?.calificacion ?? 0);
  const [descripcion, setDescripcion] = useState(reporte?.descripcion ?? "");
  const [foto, setFoto] = useState<string | null>(reporte?.foto ?? null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = esEdicion
        ? await fetch(`/api/mapa/reportes/${reporte.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
        : await fetch(`/api/mapa/obras/${obra.id}/reportes`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              ...payload,
      ...
      toast({
        title: esEdicion ? "Reporte actualizado" : "Reporte enviado",
        description: esEdicion
          ? "Los cambios quedaron guardados."
          : (json.message ?? "Gracias por aportar."),
      });
      onEnviado();
      onClose();
    } catch (error) {
      toast({
        variant: "destructive",
        title: esEdicion ? "No se pudo guardar el reporte" : "No se pudo enviar el reporte",
      ...
      <DialogContent className="max-w-lg sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogTitle className="text-xl font-bold leading-tight text-tinta">
            {esEdicion ? "Editar reporte" : "¿Y tú cómo la ves?"}
          </DialogTitle>
          <DialogDescription className="line-clamp-2 text-sm text-tinta/65">
            {esEdicion
              ? "Corrige el estado, la calificación o la foto registrada."
          ...
          <div className="flex flex-col justify-between gap-3 rounded-[4px] border border-dashed border-tinta/25 bg-papel p-3 sm:flex-row sm:items-center">
            ...
            {esEdicion && foto ? (
              <button
                ...
                className="rounded-md px-2 py-1.5 text-xs font-medium text-red-600 transition-colors hover:bg-red-50"
              >
                Quitar foto
              </button>
            ) : null}
